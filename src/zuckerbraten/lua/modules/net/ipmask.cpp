...
{
    zuckerbraten::ip::address_prefix value;
    ...
    if(arg1_type == LUA_TSTRING)
    {
        try
        {
            value = zuckerbraten::ip::address_prefix::parse(lua_tostring(L, 1));
        }
        catch(std::bad_cast)
        {
    ...
    else if(arg1_type == LUA_TNUMBER)
    {
        ...
        value = zuckerbraten::ip::address_prefix(zuckerbraten::ip::address(prefix), bits);
    }
...
int ipmask_table::__index(lua_State * L)
{
    ...
    zuckerbraten::ip::address_prefix key;
    
    if(lua_type(L, 2) == LUA_TSTRING)
    {
        try
        {
            key = zuckerbraten::ip::address_prefix::parse(lua_tostring(L, 2));
        }
        catch(std::bad_cast)
        {
    ...
    zuckerbraten::ip::address_prefix_tree<int, -1> * child = &self->tree;
...
int ipmask_table::__newindex(lua_State * L)
{
    ...
    else if(arg2_type == LUA_TNUMBER)
    {
        ...
        key = zuckerbraten::ip::address_prefix(zuckerbraten::ip::address(prefix), 32);
    }
    else key = *lua::to<ipmask>(L, 2);
